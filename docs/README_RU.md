...
# Позволить audio8bit самому выбрать все настройки для этой песни (выводит, что выбрано)
audio8bit -i song.mp3 --auto

# Авто, но с принудительной настройкой - ваш флаг побеждает, остальное остается автоматическим
...
- `--auto` - автоматически подбирает `source`, `method`, `voices`, `transpose` и `duty` по песне (любой флаг, который вы также укажете явно, побеждает; выбранные настройки выводятся на экран)

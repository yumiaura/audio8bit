...
# マルチ楽器バンド: パルス・リード + パルス・ハーモニー + 三角波ベース
...
- `-V, --voices` - `chords`(ハーモニーあり)または `lead`(単旋律) または `band`(マルチ楽器: パルス・リード + パルス・ハーモニー + 三角波ベース)(デフォルト: `chords`)
...
- **ポリフォニックなトランスクリプション**(basic-pitch)により、コードやベースを保持したり、単一のリード旋律に減らしたりできます。 `band` モードはそれらをチップのチャンネルに振り分けます(パルス・リード、パルス・ハーモニー、三角波ベース)。

...
- `-V, --voices` - `chords`(ハーモニーあり)または `lead`(単旋律) または `band`(マルチ楽器: パルス・リード + パルス・ハーモニー + 三角波ベース + ノイズ・ドラム)(デフォルト: `chords`)
...
- **ポリフォニックなトランスクリプション**(basic-pitch)により、コードやベースを保持したり、単一のリード旋律に減らしたりできます。 `band` モードはそれらをチップのチャンネルに振り分けます(パルス・リード、パルス・ハーモニー、三角波ベース)。 ベースはベース・ステム、ドラムはドラム・ステムから取り込みます。

...
# マルチ楽器バンド: パルス・リード + パルス・ハーモニー + 三角波ベース + ノイズ・ドラム
audio8bit -i song.mp3 -V band

# NES風: アルペジオ・ハーモニーのバンド、ビートに整列
...
- `--key-snap` - band/nes: 調から外れた音を検出したキーに合わせます: `on`, `off`(デフォルト: `on`)
- `--arrange` - band/nes: 転写の再生ではなくコード進行の伴奏、ルート音のベース、ループするドラムを使います: `on`, `off`(デフォルト: `on`)
- `--echo` - band/nes: メロディへのテンポ同期エコー: `on`, `off`(デフォルト: `on`)
- `--dither` - band/nes: ビット量子化前の TPDF ディザ: `on`, `off`(デフォルト: `on`)
...
- **ポリフォニックなトランスクリプション**(basic-pitch)により、コードやベースを保持したり、単一のリード旋律に減らしたりできます。 `band` モードはそれらをチップのチャンネルに振り分けます(パルス・リード、パルス・ハーモニー、三角波ベース)。 ベースはベース・ステム、ドラムはドラム・ステムから取り込みます。 `nes` モードはハーモニーをアルペジオにし、ビートに整列させます。

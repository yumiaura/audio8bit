...
- `-V, --voices` - `chords` (화음 포함) 또는 `lead` (단일 라인) 또는 `band` (멀티 악기: 펄스 리드 + 펄스 하모니 + 삼각파 베이스) (기본값: `chords`)
...
- **다성 전사**(basic-pitch)로 코드와 베이스를 유지하거나, 단일 리드 라인으로 줄여줍니다. `band` 모드는 이를 칩 채널로 나눕니다 (펄스 리드, 펄스 하모니, 삼각파 베이스).

...
# picks vocals when the song actually has a vocal and the instrumental otherwise.
SOURCE_AUTO = "auto"
...
def separate_sources(path, use_cache=True, cache_dir=None):
    """Split a song into Demucs stems.

    Returns ``(stems, sample_rate)`` where ``stems`` maps each source name
    (``drums``, ``bass``, ``other``, ``vocals``) to a mono float32 signal.

    When ``use_cache`` is true, the separated stems are cached by:
    - SHA-256 of the input file
    - Demucs model name
    - separation seed
    - Demucs shift setting

    This means changing render settings like --bits, --rate, --duty,
    --transpose, or --voices reuses the expensive Demucs output.
    """
    ...
    if use_cache:
        try:
            ...
            # Cache failure should never break a conversion.
            pass
...
def write_wav16(path, signal, sample_rate):
    """Write a mono float signal as a 16-bit signed PCM WAV."""
...
def transcribe_notes(signal, sample_rate):
    """Transcribe a possibly polyphonic signal into note events.
...
def melody_register(events):
    ...
    pitches = np.array([pitch for _start, _end, pitch, _amplitude in events])
    weights = np.array(
        [
            max(1, int((end - start) * amplitude * 100))
            for start, end, _pitch, amplitude in events
        ]
    )
...
def frame_candidates(events, frames, frame, low, high):
    """For each time frame, return in-band notes sounding then."""
...
def melody_path(
    candidates,
    smoothing=MELODY_SMOOTHING,
    rest_level=MELODY_REST_LEVEL,
    rest_switch=MELODY_REST_SWITCH,
):
    """Viterbi path: the smoothest loud line through candidates, with rests."""
    ...
    scores = [reward for _pitch, reward in states[0]]
    ...
    for index in range(1, len(states)):
        ...
        for pitch, reward in states[index]:
            ...
            for prev_index, (prev_pitch, _prev_reward) in enumerate(previous):
...
def melody_line(events, frame=MELODY_FRAME, min_seconds=MELODY_MIN_SECONDS, merge_gap=MELODY_MERGE_GAP):
    ...
    end = max(end_time for _start, end_time, _pitch, _amplitude in events)
...
def render_melody(notes, sample_rate, duty=DEFAULT_DUTY):
    """Render a monophonic note line as a clean chip lead."""

    span = max(start + duration for start, duration, _midi in notes)
...
def smooth_envelope(signal, window):
    """Centered moving average of abs(signal) over window samples."""
...
def soft_limiter(signal, sample_rate, threshold=LIMITER_THRESHOLD, window_ms=LIMITER_WINDOW_MS):
    """Tame loud stretches with a smooth gain curve."""
...
def render_chords(events, sample_rate, duty=DEFAULT_DUTY, transpose=0, min_seconds=CHORD_MIN_SECONDS):
    """Render every transcribed note so harmony and bass are kept."""

    notes = [event for event in events if event[1] - event[0] >= min_seconds]
    if not notes:
        notes = list(events)

    span = max(end for _start, end, _pitch, _amplitude in notes)
    total = int(span * sample_rate) + sample_rate
    buffer = np.zeros(total, dtype=np.float64)
...
def smooth_pitch(f0, voiced, window=5):
    """Median-filter voiced f0 values to suppress octave jumps and spikes."""
...
def segment_notes(f0, voiced, hop, analysis_rate, tolerance=PITCH_TOLERANCE, drift_frames=DRIFT_FRAMES):
    """Split a pitch track into notes with hysteresis."""
    ...
    for index in range(f0.size):
        ...
        if abs(value - center) <= tolerance:
            for _held_index, held_value in pending:
...
def merge_notes(notes, bridge_seconds=BRIDGE_SECONDS, tolerance=PITCH_TOLERANCE):
    """Bridge short gaps between neighbouring notes of similar pitch."""
...
def absorb_trills(notes, max_middle_seconds=0.15, max_gap_seconds=0.12):
    ...
    while index < len(notes):
        if out and index + 1 < len(notes):
            ...
            if (
                round(previous[2]) == round(following[2])
                ...
                previous[2] = (
                    previous[2] * previous[1] + following[2] * following[1]
                ) / weight
...
def fold_octaves(notes, span=OCTAVE_FOLD_SEMITONES, context_seconds=CONTEXT_SECONDS):
    """Fold pitch-tracking octave errors back toward the local melody line."""
    ...
    for index, note in enumerate(notes):
        ...
        weights = np.array(
            [
                ...
                for other, is_near in zip(notes, near)
                if is_near
            ]
        )
        pitches = np.array(
...
def collapse_octaves(notes, limit=OCTAVE_COLLAPSE_SEMITONES):
    """Fold octave outliers near the duration-weighted melody center."""
...
def decode_mix(path, rate):
    """Decode the original mix to mono float32 at rate through ffmpeg."""
...
def track_beats(mix, mix_rate):
    """Estimate tempo and beat times of the original mix."""
...
def quantize_notes(notes, tempo, beat_times, snap_seconds=GRID_SNAP_SECONDS):
    """Snap onsets to eighth grid and durations to sixteenths."""
...
def normalize_register(notes, transpose):
    """Shift melody by octaves into the ringtone register, then transpose."""
...
def harmonics_below_nyquist(top_frequency, sample_rate):
    """How many harmonics fit under the alias guard band."""
...
def band_limited_pulse(phase, top_frequency, sample_rate, duty=DEFAULT_DUTY):
    """Sum only pulse harmonics that fit below Nyquist."""
...
def chip_envelope(count, sample_rate, attack=0.004, decay=0.12, sustain=0.55, release=0.025):
    """Plucky chip envelope: fast attack, exponential decay, short release."""
...
def add_echo(signal, delay_samples, feedback=ECHO_FEEDBACK):
    ...
    for block in range(delay_samples, out.size, delay_samples):
        ...
        out[block:end] += feedback * out[
            block - delay_samples : block - delay_samples + end - block
        ]
...
def render_song(notes, sample_rate, duty, tempo):
    """Render pulse lead plus triangle bass plus tempo-synced echo."""
...
def alias_share(samples_u8, sample_rate):
    """Fraction of spectral energy above the band-limit guard."""
...
def validate_melody(notes, samples_u8, sample_rate):
    """Score a monophonic result against melody heuristics."""
    ...
    check("median note length", f"{median_duration * 1000:.0f} ms", median_duration >= 0.12, ">= 120 ms")

    short_share = float((durations < 0.1).mean())
    check("notes under 100 ms", f"{short_share * 100:.0f}%", short_share < 0.1, "< 10%")

    trills = sum(
        1
        for index in range(len(notes) - 2)
    ...
    check("trill flicker", f"{trills} ({trill_share * 100:.0f}%)", trill_share < 0.05, "< 5%")
    ...
    if len(notes) > 1:
        ...
        jump_share = 0.0
    check("leaps over a fifth", f"{jump_share * 100:.0f}%", jump_share < 0.3, "< 30%")

    span = float(starts[-1] + durations[-1] - starts[0]) if len(notes) else 0.0
    coverage = sounding / span if span else 0.0
    check("sound coverage", f"{coverage * 100:.0f}%", 0.12 <= coverage <= 0.97, "12-97%")

    aliasing = alias_share(samples_u8, sample_rate)
    check("energy above the band limit", f"{aliasing * 100:.2f}%", aliasing < 0.01, "< 1%")
    ...
    return all(ok for ok, _text in checks), lines


def validate_audio(samples_u8, sample_rate, note_count):
    """Audio-level checks for polyphonic/chord rendering."""
...
def quantize(samples, bits):
    """Quantize [-1, 1] float samples to bits levels."""
...
def to_uint8(samples):
    """Map [-1, 1] float samples to 8-bit unsigned PCM bytes."""
...
def write_wav(path, samples_u8, sample_rate, channels):
    """Write interleaved uint8 samples as an 8-bit unsigned PCM WAV."""
...
def reencode(wav_path, output_path):
    """Re-encode a WAV into another container/codec chosen by extension."""
...
def write_output(destination, samples_u8, sample_rate, channels):
    """Write 8-bit samples to destination, re-encoding when not WAV."""
...
def extract_notes(f0, voiced, analysis_rate):
    """Convert pitch track to a cleaned note list."""
...
def extract_by_pitch(signal, sample_rate, fmin, fmax, picked, transpose, origin):
    """pYIN pitch track path, then snap notes onto the song beat grid."""
...
def convert(
    ...
    ``source`` chooses ``vocals`` / ``instrumental`` / ``auto``.
    ``method`` chooses ``transcribe`` / ``pitch``.
    ``voices`` applies to ``transcribe`` and chooses ``chords`` / ``lead``.
    ...
    if not -24 <= transpose <= 24:
        raise ConversionError(f"--transpose must be between -24 and 24 semitones, got {transpose}")
    if source not in SOURCE_CHOICES:
        raise ConversionError(f"--source must be one of {', '.join(SOURCE_CHOICES)}, got '{source}'")
    if method not in METHOD_CHOICES:
        raise ConversionError(f"--method must be one of {', '.join(METHOD_CHOICES)}, got '{method}'")
    if voices not in VOICES_CHOICES:
        raise ConversionError(f"--voices must be one of {', '.join(VOICES_CHOICES)}, got '{voices}'")

    destination = resolve_output_path(origin, output_path, format)

    stems, sample_rate = separate_sources(
        origin,
        use_cache=use_cache,
        cache_dir=cache_dir,
    )
